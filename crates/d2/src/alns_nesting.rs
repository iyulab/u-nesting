                        ...
                        clamp_placement_to_boundary(x, y, geom_aabb, boundary_aabb)
                    {
                        ...
                        if earlier {
                            best_placement = Some(PlacedItem {
                                ...
                                rotation,
                                mirrored: mirror,
                                score: axis.key((clamped_x, clamped_y)).0,
                            });
                        }
                    }
                }
...
mod tests {
    ...
    /// `allow_flip`/mirroring, ALNS strategy. `try_place_item` calls no
    /// `.validate()`, so calling it directly genuinely bypasses the public
    /// `allow_flip = true` rejection — same bypass strategy as GA/SA/
    /// BRKGA/GDRR.
    #[test]
    fn test_alns_try_place_item_mirror_no_overlap() {
        ...
        let problem = AlnsNestingProblem::new(
            geometries.clone(),
            boundary,
            config,
    ...
    #[test]
    fn test_alns_try_place_item_mirror_ignored_without_allow_flip() {
