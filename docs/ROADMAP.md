...
#### 산출물
- [x] `d2/nfp_cm_solver.rs` - NFP-CM MILP exact solver. **2026-08 갱신(0.8.1)**: 배선이
  살아난(위 8.1 항목) 뒤에야 드러난 별도 결함 — 이 솔버는 선택된 후보의 그리드 앵커
  좌표를 회전된 로컬 프레임 오프셋 보정 없이 그대로 `Placement`의 원점으로 사용하고
  있었음(다른 모든 전략은 이 보정을 이미 적용). 축-정렬 사각형은 그 오프셋이 우연히
  0이라 정상 배치됐지만, 그 외 모든 형상(오목/볼록 불문, 솔버가 strip length 최소화를
  위해 고른 회전 각도가 0이 아닌 경우 포함)은 경계 밖으로 잘못 배치돼 조용히
  미배치 처리됨. 다른 전략과 동일한 anchor-to-origin 보정을 적용해 수정. **같은
  0.8.1에서 두 번째 결함도 발견·수정**: 위 결함이 다중-인스턴스 배치 자체를 막고
  있었기 때문에 그동안 가려져 있던 것 — 두 후보 간 충돌 검사가 기준 조각이 자기
